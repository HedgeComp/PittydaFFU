...
CheckPath -keypath $regPath
Set-ItemProperty -Path $regPath -Name "Protectyourpc" -Value 3 -Type DWord -Force -ErrorAction SilentlyContinue
#HideEULAPage 
...
# STEP 7: Set OOBE Protect your PC to Value 3
reg.exe add "HKU\Default\Software\Microsoft\Windows\CurrentVersion\Privacy" /v ProtectYourPC /t REG_DWORD /d 3 /f

# STEP 7A: Stop Start menu from opening on first logon
...
#<this gerys out Search Higlight reg.exe add "HKU\Default\Software\Policies\Microsoft\Windows\Explorer" /v DisableSearchBoxSuggestions /t REG_DWORD /d 1 /f  # | Out-Null
reg.exe delete "HKU\Default\Software\Policies\Microsoft\Windows\Explorer" /v DisableSearchBoxSuggestions /f
reg.exe add "HKU\Default\Software\Microsoft\Windows\CurrentVersion\SearchSettings" /v IsDynamicSearchBoxEnabled /t REG_DWORD /d 0 /f
